...
public class Semantic{
	...
	public Semantic(AST.program program){
      ...
      AST.class_ Main_ = str2class.get("Main");
      if(Main_ != null) { 
	 boolean has_main = false;
	 for (AST.feature f:Main_.features) 
		 if (f instanceof AST.method && f.name.equals("main")) {
			 has_main=true;
			 if (!((AST.method)f).formals.isEmpty()) 
				 reportError(Main_.filename, Main_.lineNo, "'main' method in class Main should "+
						 "have no arguments");
		 }
	 if (!has_main)
	         reportError(Main_.filename,Main_.lineNo,"No 'main' method in class Main.");
      }
	...
	public void checkNameScope(AST.class_ node)
	{
		...
		for (AST.feature feature:node.features) {
			...
			if (feature instanceof AST.method) {
               ...
               if(str2class.get(feature.typeid) == null && !feature.typeid.equals("SELF_TYPE"))
                  reportError(node.filename,feature.lineNo, "Undefined return type "+feature.typeid+" in method "+feature.name);
